...
def main():
    ...
    print("\n--- [Task 1] Fixed Goal Optimization with MLP Init (LBFGS, Rmat Physics) ---")

    # 1. MLP Inference (Warm Start)
    inference_start = time.time()

    # Rmat 학습 스크립트(train_mlp_rmat.py)에서 저장한 가중치 경로와 동일하게 맞춤
    ...
    with torch.no_grad():
        ...
        plot_trajectory(
            ...
            q_dot_traj_single,
            euler_traj,
            f"MLP+LBFGS Rmat (Err: {final_loss:.6f})",
            os.path.join(save_dir, "mlp_lbfgs_traj_rmat.png"),
            TOTAL_TIME,
