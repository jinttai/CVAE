...
import ddp.src.dynamics_torch as dynamics  # noqa: E402
import ddp.src.cost as cost  # noqa: E402
import ddp.src.solver as solver  # noqa: E402
from ddp.src.trajectory_utils import save_trajectory_csv, analyze_trajectory_fourier  # noqa: E402
...
def quat_to_euler(q):
    ...
    Returns angles in radians. Expects numpy array.
    """
    x, y, z, w = q
...
def main():
    ...
    # ------------------------------------------------------------------
    # 4. Save results and plots under ddp/results (suffix _nn to distinguish)
    # ------------------------------------------------------------------
    ...
    axes[1, 0].plot(time_arr, euler_angles)
    ...
    csv_filename = f"trajectory_{method_name.lower()}_nn.csv"
    csv_path = os.path.join(save_dir, csv_filename)
    save_trajectory_csv(states, controls, dt, csv_path, method_name=f"{method_name.lower()}_nn")
    
    # Perform Fourier analysis
    analyze_trajectory_fourier(
        states, controls, dt, save_dir, 
        method_name=f"{method_name.lower()}_nn", 
        plot=True, 
        save_csv=True
    )
