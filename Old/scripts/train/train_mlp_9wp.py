"""
MLP Training with 9 waypoints / 200 timesteps.
...
def main():
    ...
    if len(train_losses) > 0:
        ...
        plt.xlabel("Epoch")
        plt.ylabel("Loss")
        plt.title("MLP 9-Waypoint Training Curve")
        plt.grid(True)
        plt.legend()
    ...
    # Save model weights
    save_dir = os.path.join(ROOT_DIR, "outputs/weights/MLP_9wp")
    os.makedirs(save_dir, exist_ok=True)
