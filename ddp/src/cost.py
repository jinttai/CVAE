...
import torch
import ddp.src.dynamics_torch as dynamics
...
class TerminalCost:
    """
    Terminal cost L_final(x, u) = orientation_cost + joint_cost + velocity_cost
    - orientation_cost: geodesic distance on SO(3) to goal orientation
    - joint_cost: squared distance to goal joint angles
    - velocity_cost: squared norm of final joint velocities (to make final velocity zero)
    """
    
    def __init__(self, goal_quaternion, goal_joints=None, 
                 orientation_weight=1.0, joint_weight=1.0, velocity_weight=0.0, device='cpu'):
        """
        Args:
            ...
            velocity_weight: weight for joint velocity cost (to make final velocity zero)
            device: device for tensors
        """
        self.device = device
    ...
    def __call__(self, state, control=None):
        """
        Compute terminal cost L_final(x, u).
        
        Args:
            state: [10] tensor [q1...q6, qx, qy, qz, qw]
            control: [6] tensor [qd1...qd6] (optional, final joint velocities)
        ...
        # Joint velocity cost: squared norm of final joint velocities (to make final velocity zero)
        if control is not None and self.velocity_weight > 0.0:
            velocity_cost = self.velocity_weight * torch.sum(control ** 2)
            total_cost = total_cost + velocity_cost
