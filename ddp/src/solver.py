...
class DDP:
    ...
    def forward_pass(self, initial_state, nominal_states, controls, k, K, dt, alpha=1.0):
        ...
        # Terminal state cost (with final control for velocity cost)
        final_control = new_controls[-1] if new_controls.shape[0] > 0 else torch.zeros(6, device=self.device)
        total_cost = total_cost + self.terminal_cost(new_states[-1], final_control)
    ...
    def solve(self, initial_state, initial_controls, dt):
        ...
        # Terminal cost with final control for velocity cost
        final_control = controls[-1] if controls.shape[0] > 0 else torch.zeros(6, device=self.device)
        initial_cost = initial_cost + self.terminal_cost(states[-1], final_control)
