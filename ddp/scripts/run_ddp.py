...
import ddp.src.solver as solver
...
def main():
    ...
    terminal_cost = cost.TerminalCost(
        ...
        velocity_weight=1.0,  # Weight for final joint velocity (to make final velocity zero)
        device=device
    )
    ...
    # Save results under ddp/results
    ...
    axes[1, 0].plot(time, euler_angles)
    ...
    csv_filename = f"trajectory_{method_name.lower()}.csv"
    csv_path = os.path.join(save_dir, csv_filename)
    save_trajectory_csv(states, controls, dt, csv_path, method_name=method_name.lower())
