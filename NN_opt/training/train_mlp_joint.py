...
def main():
    ...
    print(f"=== MLP Joint Training Start on {device} ===")
    ...
    log_dir = os.path.join(ROOT_DIR, "outputs/logs/mlp_joint_v1")
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        # Inference (MLP forward pass - no latent variable z)
    ...
    if len(train_losses) > 0:
        ...
        csv_dir = os.path.join(plots_dir, "mlp_joint_training_curve")
        ...
        plt.xlabel("Epoch")
        plt.ylabel("Loss")
        plt.title("MLP Joint Training Curve")
        plt.grid(True)
        plt.legend()
        save_dir = os.path.join(plots_dir, "mlp_joint_training_curve")
    ...
    # 모델 저장 (전용 디렉토리)
    save_dir = os.path.join(ROOT_DIR, "outputs/weights/mlp_joint")
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)
