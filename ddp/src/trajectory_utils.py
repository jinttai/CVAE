...
def save_trajectory_csv(states, controls, dt, save_path, method_name="ilqr"):
    ...
    Args:
        states: [T+1, 10] tensor or numpy array [joint_angles (6), quaternion (4)]
        controls: [T, 6] tensor or numpy array [joint_velocities (6)]
    ...
    for i in range(6):
        data[f'joint_{i+1}_velocity'] = controls_padded[:, i]
    
    # Quaternion (states[:, 6:])
    data['quaternion_x'] = states[:, 6]
    data['quaternion_y'] = states[:, 7]
    data['quaternion_z'] = states[:, 8]
    data['quaternion_w'] = states[:, 9]
